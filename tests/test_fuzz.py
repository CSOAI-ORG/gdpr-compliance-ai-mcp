...
from server import classify_processing


# Strategy for the primary description-like string argument.
processing_description_arg = st.text(min_size=0, max_size=2000)


@given(arg=processing_description_arg)
def test_classify_processing_never_unhandled_exception(arg):
    ...
    """
    try:
        result = classify_processing(arg)
